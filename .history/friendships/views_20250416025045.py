...
class FriendListView(APIView):
    ...
    def get(self, request):
        ...
        friends_sent = Friendship.objects.filter(request_from=user, is_accepted=True).values_list('request_to', flat=True)

        friends_received = Friendship.objects.filter(request_to=user, is_accepted=True).values_list('request_from', flat=True)
