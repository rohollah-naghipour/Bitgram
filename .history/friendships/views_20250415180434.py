...
class SendFriendRequestView(APIView):
    def post(self, request, receiver_id):
        ...
        if Friendship.objects.filter(request_from=sender, request_to=user_id).exists():
            return Response({'message': 'Friend request has already been sent.'}, status=status.HTTP_409_CONFLICT)
        elif Friendship.objects.filter(request_from=user_id, request_to=sender).exists():
        ...
        Friendship.objects.create(request_from = sender, request_to = user_id)
